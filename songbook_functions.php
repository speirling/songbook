...
function sbk_get_song_title($song_id) {
    $song = acradb_get_single_record('music_admin', 'lyrics', 'id', $song_id);
    return $song['title'];
}

function sbk_add_song_titles($playlist_simplexml) {
    foreach($playlist_simplexml->song as $this_song) {
        sbk_get_song_title($this_song['id']);
        $this_song->addAttribute('titel', $this_song_title);
    }
    return $playlist_simplexml;
}

function sbk_convert_simpleXML_playlist_to_json_string($playlist_simplexml) {
    $text_as_attributes = sbk_playlist_text_to_attributes($playlist_simplexml);
...
function sbk_get_all_songs($search_string = false) {
        $searchWHERE = '';
        if($search_string) {
            $searchWHERE = $searchWHERE.' WHERE ';
            $searchWHERE = $searchWHERE.' `id` LIKE \'%'.$search_string.'%\' OR';
            $searchWHERE = $searchWHERE.' `title` LIKE \'%'.$search_string.'%\' OR';
            $searchWHERE = $searchWHERE.' `written_by` LIKE \'%'.$search_string.'%\' OR';
            $searchWHERE = $searchWHERE.' `performed_by` LIKE \'%'.$search_string.'%\' OR';
            $searchWHERE = $searchWHERE.' `content` LIKE \'%'.$search_string.'%\' OR';
        ...
        while ($this_record = mysql_fetch_assoc($result)) {
            ...
            $output_json = $output_json."\"credits\" : {\"written_by\": \"".str_replace('"', '&quot;', trim($this_record["written_by"]))."\", \"performed_by\": \"".str_replace('"', '&quot;', trim($this_record["performed_by"]))."\"}, ";
            $output_json = $output_json."\"introduction\":{\"duration\":\"\", \"text\":\"\"}";
            $output_json = $output_json."}, ";
    	}
        //remove trailing comma and space
        $output_json = substr($output_json, 0, -2);
        $output_json = "[".$output_json."]";
...
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// Obsolete - should be done in javascript by the client ----------------------
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
// ----------------------------------------------------------------------------
